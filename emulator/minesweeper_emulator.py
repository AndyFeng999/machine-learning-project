...
def get_bombs(x, y, board):
    neighbors = []
    if is_in_range(x-1, y-1) and board[x-1][y-1] == -9:
        neighbors.append((x-1, y-1))
    if is_in_range(x-1, y) and board[x-1][y] == -9:
        neighbors.append((x-1, y))
    if is_in_range(x-1, y+1) and board[x-1][y+1] == -9:
        neighbors.append((x-1, y+1))
    if is_in_range(x, y+1) and board[x][y+1] == -9:
        neighbors.append((x, y+1))
    if is_in_range(x+1, y+1) and board[x+1][y+1] == -9:
        neighbors.append((x+1, y+1))
    if is_in_range(x+1, y) and board[x+1][y] == -9:
...
def generate_solved_board(height, width, bombs):
    global width_master 
    width_master = width
    global height_master
    height_master = height
    ...
    while (h1 != height):
        while (w1 != width):
            if (board[h1][w1] != -9):
                neighbors = get_bombs(h1,w1,board)
                board[h1][w1] = len(neighbors)
            w1 = w1 + 1
        h1 = h1 + 1
        w1 = 0
    print(board)
    
generate_solved_board(16,16,40)
